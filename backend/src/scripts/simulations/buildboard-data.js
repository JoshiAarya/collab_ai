...
export const stage1_ideation = [
  ...
  { thread: 'Main', user: 'Nikunj', text: 'We can use JSONB columns in Postgres for that — we get both relational integrity and document flexibility.' },
  ...
  { thread: 'Main', user: 'Nikunj', text: 'AWS makes the most sense since our organization\'s existing billing/tooling is all there.' },
...
export const stage2_planning = [
  ...
  { thread: 'Backend', user: 'Prathamesh', text: 'That makes total sense. Postgres with JSONB it is.' },
  ...
  { thread: 'Backend', user: 'Nikunj', text: 'I prefer short-lived JWTs paired with refresh tokens stored in httpOnly secure cookies. It mitigates XSS and CSRF risks while maintaining stateless scalability.' },
  ...
  { thread: 'Frontend', user: 'Nikunj', text: 'Agreed on Vite. What about frontend state management? Redux?' },
  { thread: 'Frontend', user: 'Digambar', text: 'Redux involves too much boilerplate. I propose Zustand for global state, and React Query specifically for server caching.' },
  { thread: 'Frontend', user: 'Adarsh', text: 'Sounds solid. And component library?' },
  { thread: 'Frontend', user: 'Digambar', text: 'Let\'s use Tailwind CSS with Shadcn UI. It gives us fully accessible components that we can heavily customize without fighting rigid default styles.' },
  ...
  { thread: 'Infrastructure', user: 'Nikunj', text: 'EKS is probably overkill for our current engineering footprint. ECS with Fargate gives us serverless Docker execution without the control plane maintenance.' },
  ...
  { thread: 'Infrastructure', user: 'Aarya', text: 'Jenkins is a headache. Let\'s establish our CI/CD pipeline purely through GitHub Actions. We can build images, push to ECR, and trigger ECS service updates.' },
  ...
  { thread: 'Integrations', user: 'Prathamesh', text: 'Let\'s resolve webhook vs polling then: we will use webhooks backed strictly by SQS instead of cron polling.' },
...
export const stage3_development = [
  ...
  { thread: 'Integrations', user: 'Nikunj', text: 'We cannot let this block compliance testing. Actually, instead of DocuSign we\'re going with HelloSign (Dropbox Sign) for MVP. I reviewed their docs and their embedded signing mechanism handles dynamic custom fields way better.' },
  ...
  { thread: 'Frontend', user: 'Digambar', text: 'Good point. I\'ll implement react-pdf with virtualized rendering so it only mounts the exact pages currently visible on screen.' },
  ...
  { thread: 'Frontend', user: 'Digambar', text: 'I can inject a mock configuration for the HelloSign SDK during standard `NODE_ENV=test` runs so it bypasses the iframe and resolves the promise directly.' },
...
export const stage4_testing = [
  ...
  { thread: 'Backend', user: 'Nikunj', text: 'Actually, this isn\'t working, moving to PostgreSQL\'s native `tsvector` full-text search capability instead of basic `ILIKE` queries. It gives us highly indexed search speeds without the massive overhead of managing an Elasticsearch cluster.' },
  ...
  { thread: 'Backend', user: 'Nikunj', text: 'I am rewriting the RBAC middleware on that controller right now to strictly verify tenant ownership before returning the resource data.' },
  ...
  { thread: 'Backend', user: 'Prathamesh', text: 'We originally chose instant transaction emails during planning, but we need to revisit this architectural decision. A daily digest batch job is much safer and less annoying.' },
