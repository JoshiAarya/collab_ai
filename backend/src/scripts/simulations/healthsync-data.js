...
export const stage2_planning = [
  ...
  { thread: 'Backend', user: 'alice', text: 'Express has a larger ecosystem and more middleware for things like rate limiting and security headers which we need right away. Let\'s stick with Express.' },
  ...
  { thread: 'Backend', user: 'alice', text: 'Given the strict financial and medical transaction nature, we need strong ACID compliance. I strongly suggest we use Postgres instead. We can use JSONB columns if we need document flexibility.' },
  ...
  { thread: 'Backend', user: 'alice', text: 'For authentication, we should use JWTs but with short expirations and httpOnly secure cookies to mitigate XSS.' },
  ...
  { thread: 'Frontend', user: 'bob', text: 'Over on the frontend, I\'m setting up the React boilerplate. We will use Vite instead of Create React App for faster builds.' },
  { thread: 'Frontend', user: 'dr_smith', text: 'Sounds good. What about styling? Should we use Tailwind CSS or styled-components?' },
  { thread: 'Frontend', user: 'bob', text: 'Let\'s adopt Tailwind CSS. It will speed up our UI development significantly, especially for the complex dashboard layouts.' },
  { thread: 'Frontend', user: 'bob', text: 'For state management, we should use Zustand. It\'s lighter than Redux and perfect for our needs.' },
  ...
  { thread: 'Infrastructure', user: 'charlie', text: 'Yes, we can deploy the LiveKit server on our own EC2 instances within a private VPC. I\'ve read through their deployment docs.' },
  ...
  { thread: 'Backend', user: 'charlie', text: 'I\'ll set up Redis. We can use BullMQ to manage the job queues reliably.' },
...
export const stage3_development = [
  ...
  { thread: 'Backend', user: 'alice', text: 'We should use WebSockets. When the patient connects, our backend will emit a `participant_joined` event down to the doctor\'s client.' },
...
export const stage4_testing = [
  ...
  { thread: 'Infrastructure', user: 'charlie', text: 'I also think we should switch from standard Redis Lists to Redis Streams for better queue management and consumer group scaling.' },
